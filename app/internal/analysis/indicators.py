...
def calculate_bollinger_bands(df: pd.DataFrame, window: int = 20, num_std_dev: int = 2) -> pd.DataFrame:
    ...
    df['bollinger_upper'] = df['bollinger_middle'] + num_std_dev * df['close'].rolling(window=window).std()
    df['bollinger_lower'] = df['bollinger_middle'] - num_std_dev * df['close'].rolling(window=window).std()
    return df
...
def calculate_vma(df: pd.DataFrame, short_window: int = 5, long_window: int = 20) -> pd.DataFrame:
